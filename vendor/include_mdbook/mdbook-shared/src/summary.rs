...
impl<'a> SummaryParser<'a> {
    ...
    fn parse_parts(&mut self) -> Result<Vec<SummaryItem<PathBuf>>> {
        ...
        loop {
            ...
            let numbered_chapters =
                self.parse_numbered(&mut root_items, &mut root_number).map_err(|err| {
                    anyhow::anyhow!("There was an error parsing the numbered chapters: {err}")
                })?;
    ...
    fn parse_link(&mut self, href: String) -> Result<Link<PathBuf>> {
        ...
        let path = if href.is_empty() {
            ...
            if let Some(src_path) = self.src_path {
                ...
                let full_path =
                    get_book_content_path(PathBuf::from(&src_path)).unwrap().join(&path_buf);
                if !full_path.exists() {
        ...
        loop {
            match self.next_event() {
                ...
                Some(Event::Start(Tag::List(..))) => {
                    ...
                    let last_item_number =
                        last_item.number.as_ref().expect("All numbered chapters have numbers");
...
mod tests {
    ...
    fn parse_a_numbered_chapter() {
        ...
        let mut parser = SummaryParser::new(None, src);
        let got = parser.parse_numbered(&mut 0, &mut SectionNumber::default()).unwrap();
