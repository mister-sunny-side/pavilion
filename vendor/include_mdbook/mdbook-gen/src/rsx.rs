        ...
        match tag {
            ...
            Tag::CodeBlock(kind) => {
                ...
                if lang.as_deref() == Some("inject-dioxus") {
                    ...
                    // Pavilion: emit plain strings instead of dioxus-code HighlightedSource
                    // so wasm builds do not require arborium's C wasm toolchain.
                    let source = raw_code.trim_end();
                    let lang_str = lang.as_deref().unwrap_or("");

                    let fname = if let Some(fname) = fname {
                        quote! { name: Some(#fname.to_string()) }
                    } else {
                        quote! { name: None }
                    };
...
fn resolve_extension(path: &Path, file: &str, fname: &mut Option<String>) -> syn::Result<String> {
    ...
    *fname = Some(PathBuf::from(file).file_name().unwrap().to_string_lossy().to_string());
